...
function calculate() {
    ...
    let chargeScore = [0, 0, 0, 0];
    for (let i = 1; i <= 4; i++) {
        chargeScore[i - 1] = scores[i - 1].calcScore;
        chargeScore[i - 1] += parseInt(document.getElementById(`chip${i}`).value) * chip;
    }
    const indices = [0, 1, 2, 3].sort((a, b) => chargeScore[b] - chargeScore[a]);
    const [firstMaxIndex, secondMaxIndex, thirdMaxIndex, fourthMaxIndex] = indices;
    ...
    const num14 = chargeScore[firstMaxIndex] - chargeScore[fourthMaxIndex];
